...
import { OrderState, FluidOrder, PartOrder, VehicleInfo, Language } from "../types";
...
- You are NOT responsible for decoding VINs or looking up license plates. A deterministic engine handles this.
- If the user provides a VIN or license plate, acknowledge it and wait for the system to process it.
- If the system finds a vehicle, it will update the 'Order' state. You should then ask the user to CONFIRM the identified vehicle (e.g., "I found a 2022 Toyota Corolla. Is that correct?").
- If the VIN or plate is invalid or ambiguous, the system will inform you. You must then ask the user for clarification or missing characters.
...
- LICENSE PLATE LOOKUP: If a license plate is identified but the state is unknown, ask the user for the state. Once the user provides a state, use the 'update_plate_state' tool. The system will then automatically look up the vehicle.
- If identification fails, fallback to asking for Year, Make, and Model manually.
...
`;
    ...
    const systemInstruction = this.customSystemInstruction || defaultSystemInstruction;
